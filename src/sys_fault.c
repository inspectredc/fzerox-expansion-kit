...
s32 idleThreadCounter;
s32 mainThreadCounter;
s32 gameThreadCounter;
s32 audioThreadCounter;
s32 unkThreadCounter;
FaultMgr gFaultMgr;
...
void Fault_DisplayDebugInfo(OSThread* thread) {
    ...
    for (idleThreadCounter = 0; ((u64*) sIdleThreadStack)[idleThreadCounter] == 0x8877665544332211;
         idleThreadCounter++) {}

    for (mainThreadCounter = 0; ((u64*) sMainThreadStack)[mainThreadCounter] == 0x8877665544332211;
         mainThreadCounter++) {}

    for (gameThreadCounter = 0; ((u64*) sGameThreadStack)[gameThreadCounter] == 0x8877665544332211;
         gameThreadCounter++) {}

    for (audioThreadCounter = 0; ((u64*) sAudioThreadStack)[audioThreadCounter] == 0x8877665544332211;
         audioThreadCounter++) {}

    for (unkThreadCounter = 0; ((u64*) D_80797670)[unkThreadCounter] == 0x8877665544332211; unkThreadCounter++) {}

    Fault_Printf(120, 220, "I%dT%dG%dA%dD%d", idleThreadCounter, mainThreadCounter, gameThreadCounter,
                 audioThreadCounter, unkThreadCounter);
