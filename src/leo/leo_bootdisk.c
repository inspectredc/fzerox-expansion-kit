...
void LeoBootGame(void* entry) {
    ...
    osInvalICache(__LeoBootGame2, 0x344);
    ...
    osWritebackDCacheAll();
    osInvalICache(__LeoBootGame3, 0x50);
...
//! @bug This function and _LeoBootGame3 are supposed to be encrypted after compile time, but they are not
void __LeoBootGame2(void* entry) {
    u8* entry2 = (u8*) entry;
    u32 i;
    s32 sp134;
    ...
    *(u8*) 0xA0000090 = (u32) (LEO_country_code & 0xFF0000) >> 0x10;
    ...
    bzero(__LeoBootGame2, 0x304);
    osWritebackDCache(__LeoBootGame2, 0x304);
