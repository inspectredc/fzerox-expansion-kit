...
void Save_LoadCourseRecord(ProfileSave* profileSaves, s32 courseIndex) {
    ...
    for (i = 0, profileSave = profileSaves; i < 2; i++) {
        if (profileSave->courses[courseIndex].checksum !=
            Save_CalculateProfileSaveCourseRecordChecksum(profileSave, courseIndex)) {
            j++;
            invalidSaveIndex = i;
        }
        profileSave++;
...
void Save_LoadEditCup2(ProfileSave* profileSaves, u8* arg1, u16* arg2) {
    ...
    for (i = 0, profileSave = profileSaves; i < 2; i++) {
        if (profileSave->editCup2.checksum != Save_CalculateSaveEditCup2Checksum(profileSave) * 1) {
            invalidSaveCount++;
            invalidSaveIndex = i;
        }
        profileSave++;
